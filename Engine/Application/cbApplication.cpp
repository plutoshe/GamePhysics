...
double eae6320::Application::cbApplication::GetElapsedSecondCount_simulation() const
{
...
void eae6320::Application::cbApplication::UpdateUntilExit()
{
	...
	auto tickCount_simulationTime_elapsedButNotYetSimulated = uint64_t( 0 );
	...
	auto tickCount_systemTime_elapsedAllowable = uint64_t( 0 );
...
void eae6320::Application::cbApplication::UpdateSimulationBasedOnTime(const float i_elapsedSecondCount_sinceLastUpdate)
{
	...
	for (size_t i = 0; i < m_gameObjects.size(); i++)
	{
		m_gameObjects[i].m_rigidBodyState.UpdateState(i_elapsedSecondCount_sinceLastUpdate);
	}
