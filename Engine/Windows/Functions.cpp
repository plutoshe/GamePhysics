...
namespace
{
	// Attempts to prevent Windows's global last error code
	// from changing before it is retrieved
	struct sMutex
	{
...
std::wstring eae6320::Windows::ConvertUtf8ToUtf16( const char* const i_string_utf8 )
{
	...
	constexpr int processEntireString = -1;
	{
		...
		EnterCriticalSection( &s_mutex_lastError.criticalSection );
	...
	wchar_t buffer[maxCharacterCount];
	{
		...
		if ( PathCanonicalizeW( buffer, ConvertUtf8ToUtf16( directory.c_str() ).c_str() ) != FALSE )
		{
			...
			int windowsErrorCode;
			{
				...
				windowsErrorCode = SHCreateDirectoryExW( noWindowIsDisplayed, buffer, useDefaultSecurityAttributes );
			}
...
bool eae6320::Windows::DoesFileExist( const char* const i_path, std::string* const o_errorMessage )
{
	...
	EnterCriticalSection( &s_mutex_lastError.criticalSection );
	HANDLE file = FindFirstFileW( ConvertUtf8ToUtf16( i_path ).c_str(), &fileData );
