...
namespace
{
	...
	// Note that this is independent of the eae6320::Graphics::VertexFormats::s3dObject struct
	// that is used by the engine to store geometry data!
	// This struct is used to store Maya-specific vertex information
	// and then output it to the human-readable source geometry file.
	struct sVertex_maya
	{
		// Position
		const float x, y, z;
	...
	// This stores any information associated with a vertex that is necessary to export the geometry.
...
MStatus eae6320::cMayaGeometryExporter::writer( const MFileObject& i_file, const MString& i_options, FileAccessMode i_mode )
{
	MStatus status;
	...
	std::vector<sMaterialInfo> materialInfo;
	{
		...
		std::vector<MObject> shadingGroups;
		{
			...
			{
				status = ProcessAllObjects( uniqueVertices, triangles, shadingGroups );
				if ( !status )
				{
					return status;
				}
			...
			{
				status = ProcessSelectedObjects( uniqueVertices, triangles, shadingGroups );
				if ( !status )
				{
					return status;
				}
		...
		// Convert the geometry information to vertex and index arrays
	...
	// Write the geometry data to the requested file
	{
		const auto filePath = i_file.resolvedFullName();
		return WriteGeometryToFile( filePath, vertexArray, indexArray, materialInfo );
	}
}
