...
eae6320::cResult eae6320::Graphics::cShader::Initialize( const std::string& i_path, const Platform::sDataFromFile& i_loadedShader )
{
	...
	// Create the vertex shader object
	{
		...
		{
			EAE6320_ASSERTF( false, "Shader %s object creation failed (HRESULT %#010x)", i_path.c_str(), d3dResult );
			eae6320::Logging::OutputError( "Direct3d failed to create the shader %s with HRESULT %#010x", i_path.c_str(), d3dResult );
